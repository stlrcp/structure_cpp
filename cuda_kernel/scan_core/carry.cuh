...
#elif __CUDACC_VER_MAJOR__ >= 9 || defined(__ILUVATAR__)
...
public:
  __device__ __host__ half() {}
  __device__ __host__ half(const half &v) : data_(v.data_) {}
  __device__ __host__ half(float v) : data_(__float2half_rn(v)) {}

  explicit __device__ __host__ half(const __half_raw &v) : data_(v.x) {}
  explicit __device__ __host__ half(bool v) : data_(__float2half_rn(float(v))) {}
  explicit __device__ __host__ half(double v) : data_(__float2half_rn(float(v))) {}
  explicit __device__ __host__ half(int v) : data_(__float2half_rn(float(v))) {}
  explicit __device__ __host__ half(unsigned int v) : data_(__float2half_rn(float(v))) {}
  explicit __device__ __host__ half(long long v) : data_(__float2half_rn(float(v))) {}
  explicit __device__ __host__ half(unsigned long long v) : data_(__float2half_rn(float(v))) {}

  __device__ __host__ operator float() const {return __half2float(data_);}
  __device__ __host__ operator __half_raw() const {__half_raw ret = {data_}; return ret;}
};
...
private:
  half  data_;
public:
  __device__ __host__ float16() {}
  __device__ __host__ float16(float v) : data_(v) {}

  explicit __device__ __host__ float16(bool v) : data_(float(v)) {}
  explicit __device__ __host__ float16(double v) : data_(v) {}
  explicit __device__ __host__ float16(int v) : data_(v) {}
  explicit __device__ __host__ float16(unsigned int v) : data_(v) {}
  explicit __device__ __host__ float16(long long v) : data_(v) {}
  explicit __device__ __host__ float16(unsigned long long v) : data_(v) {}

  explicit __device__ __host__ float16(const half &v): data_(v) {}
  explicit __device__ __host__ float16(const __half_raw &v): data_(v) {}

  __device__ __host__ operator float() const {return float(data_);}

  static const unsigned short nan = 0x7e00u;

  __device__ __host__ int iszero() const {
    return (__half_raw(data_).x & 0x7fffu) == 0;
  }

  __device__ __host__ int isnan() const {
    __half_raw raw_ = __half_raw(data_);
    return (raw_.x & 0x7c00u) == 0x7c00u && (raw_.x & 0x03ffu) != 0x0000u;
  }

  __device__ __host__ int isinf() const {
    return (__half_raw(data_).x & 0x7fffu) == 0x7c00u;
  }

  __device__ __host__ int isfinite() const {
    return (__half_raw(data_).x & 0x7c00u) != 0x7c00u;
  }

  __device__ __host__ int signbit() const {
    return (__half_raw(data_).x & 0x8000u) != 0;
  }

#ifdef __HIPCC__

__device__ __host__ float16 operator-() {
  ...
  template<typename T>
  inline __device__ __host__ float16& operator+=(const T& rhs) {
    *this = *this + rhs;
    return *this;
  }
  ...
  friend __device__ __host__ float16 copysign(float16 x, float16 y) {
  ...
  friend __device__ __host__ float16 nextafter(float16 x, float16 y) {
    __half_raw x_raw_ = __half_raw(x.data_);
    __half_raw y_raw_ = __half_raw(y.data_);
    __half_raw ret_raw_;
...
private:
  static __device__ __host__ int eq_nonan(const float16 x, const float16 y) {
    __half_raw x_raw_ = __half_raw(x.data_);
    __half_raw y_raw_ = __half_raw(y.data_);
...
__device__ __host__ float16 min(float16 x, float16 y) {
  return float16(fmin(float(x), float(y)));
}
__device__ __host__ float16 max(float16 x, float16 y) {
  return float16(fmax(float(x), float(y)));
}
__device__ __host__ float16 fmin(float16 x, float16 y) {
  return float16(fmin(float(x), float(y)));
}
__device__ __host__ float16 fmax(float16 x, float16 y) {
  return float16(fmax(float(x), float(y)));
}
__device__ __host__ int iszero(float16 x) {return x.iszero();}
__device__ __host__ int isnan(float16 x) {return x.isnan();}
__device__ __host__ int isinf(float16 x) {return x.isinf();}
__device__ __host__ int isfinite(float16 x) {return x.isfinite();}
__device__ __host__ int signbit(float16 x) {return x.signbit();}
...
public:
  ...
  __device__ __host__ CArray(T* data, const Int1* shape, const Int2* strides)
      : data_(data), size_(1)
  {
    if (_c_contiguous) {
  ...
  __device__ __host__ CArray(typename cupy::type_traits::enable_if<_c_contiguous, U>::type* data,
                    const Int* shape)
      : data_(data), size_(1)
  {
    for (int i = 0; i < _ndim; i++) {
  ...
  template <typename Int1, typename Int2>
  __device__ __host__ CArray(T* data,
  ...
  __device__ __host__ CArray() : data_(NULL), size_(1)
  {
    memset(this->shape_, 0, sizeof(this->shape_));
    memset(this->strides_, 0, sizeof(this->strides_));
  }

  __device__ __host__ ptrdiff_t size() const {
    return size_;
  }

  __device__ __host__ const ptrdiff_t* shape() const {
    return shape_;
  }

  __device__ __host__ const ptrdiff_t* strides() const {
    return strides_;
  }

#ifdef CUPY_JIT_MODE
  __device__ __host__ typename cupy::as_tuple<_ndim, ptrdiff_t>::type get_shape() const {
    return cupy::as_tuple<_ndim, ptrdiff_t>::call(shape_);
  }

  __device__ __host__ typename cupy::as_tuple<_ndim, ptrdiff_t>::type get_strides() const {
  ...
  __device__ __host__ T& operator[](const std::initializer_list<Int> idx_) {
  ...
  template <typename Int>
  __device__ __host__ const T& operator[](const std::initializer_list<Int> idx_) const {
    assert(idx_.size() == _ndim);
    Int idx[ndim];
    memcpy(idx, idx_.begin(), ndim*sizeof(Int));
    return this->operator[](idx);
  ...
  template <typename Int>
  __device__ __host__ T& operator[](const Int (&idx)[ndim]) {
    return const_cast<T&>(const_cast<const CArray&>(*this)[idx]);
  }

  template <typename Int>
  __device__ __host__ const T& operator[](const Int (&idx)[ndim]) const {
    index_t diff = 0;
  ...
  __device__ __host__ T& operator[](ptrdiff_t i) {
    return const_cast<T&>(const_cast<const CArray&>(*this)[i]);
  }

#ifdef CUPY_JIT_MODE
  template <typename Tuple, int dim>
  __forceinline__ __device__ __host__ const T& _indexing(const Tuple &idx, Dim<dim>, const char* ptr) const {
  ...
  __forceinline__ __device__ __host__ const T& _indexing(const Tuple &idx, Dim<_ndim>, const char* ptr) const {
    return *reinterpret_cast<const T*>(ptr);
  }

  template <typename Tuple>
  __forceinline__ __device__ __host__ const T& _indexing(const Tuple &idx) const {
    const char* ptr = reinterpret_cast<const char*>(data_);
    return _indexing(idx, Dim<0>(), ptr);
  }

  template <typename Tuple>
  __forceinline__ __device__ __host__ T& _indexing(const Tuple &idx) {
    return const_cast<T&>(const_cast<const CArray&>(*this)._indexing(idx));
  }
#endif

  __device__ __host__ const T& operator[](ptrdiff_t idx) const {
    if (c_contiguous) {
...
public:
  ...
  __device__ __host__ explicit CArray(T* data) : data_(data), size_(1) { }

  template <typename Int>
  __device__ __host__ CArray(T* data, Int size) : data_(data), size_(size) { }
  ...
  template <typename U>
  __device__ __host__ T& operator[](const U&) {
    return *data_;
  }

  template <typename U>
  __device__ __host__ T operator[](const U&) const {
    return *data_;
  }
};
...
public:
  ...
  template <typename Int>
  __device__ __host__ explicit CIndexer(const Int* shape)
      : size_(1) {
    for (int i = 0; i < _ndim; i++) {
      this->size_ *= shape[i];
      this->shape_[i] = shape[i];
  ...
  template <typename Int1, typename Int2>
  __device__ __host__ CIndexer(const Int1* shape, const Int2* index)
      : size_(1) {
    for (int i = 0; i < _ndim; i++) {
      this->size_ *= shape[i];
      this->shape_[i] = shape[i];
...
#if __cplusplus >= 201103 || (defined(_MSC_VER) && _MSC_VER >= 1900)
  template <typename Int>
  __device__ __host__ CIndexer(const std::initializer_list<Int> shape)
  ...
  template <typename Int1, typename Int2>
  __device__ __host__ CIndexer(const std::initializer_list<Int1> shape,
  ...
  __device__ __host__ CIndexer() : size_(1) 
  {
    memset(this->shape_, 0, sizeof(this->shape_));
  ...
  __device__ __host__ const index_t& get() const {
    return index_;
  }

  __device__ __host__ void set(ptrdiff_t i) {
...
private:
  ...
  __device__ __host__ void _set(index_t i) {
      for (int dim = ndim; --dim > 0; ) {
...
public:
  ...
  template <typename Int>
  __device__ __host__ explicit CIndexer(Int size) : size_(size) { }

  // These constructors are just to match the non-0-dim constructors
  template <typename Int>
  __device__ __host__ explicit CIndexer(const Int* shape) : size_(1) { }

  template <typename Int1, typename Int2>
  __device__ __host__ CIndexer(const Int1* shape, const Int2* strides)
      : size_(1) { }
  ...
  __device__ __host__ const ptrdiff_t* get() const {
    return NULL;
  }
};

__device__ __host__ int _floor_divide(int x, int y) {
  if (y == 0) return 0;
  int q = x / y;
  return q - (((x < 0) != (y < 0)) && q * y != x);
}

__device__ __host__ long long _floor_divide(long long x, long long y) {
  if (y == 0) return 0;
  long long q = x / y;
  return q - (((x < 0) != (y < 0)) && q * y != x);
}

__device__ __host__ unsigned _floor_divide(unsigned x, unsigned y) {
  if (y == 0) return 0;
  return x / y;
}

__device__ __host__ unsigned long long _floor_divide(
    unsigned long long x, unsigned long long y) {
  if (y == 0) return 0;
  return x / y;
}

__device__ __host__ float _floor_divide(float x, float y) {
  return floor(x / y);
}

__device__ __host__ double _floor_divide(double x, double y) {
  return floor(x / y);
}
