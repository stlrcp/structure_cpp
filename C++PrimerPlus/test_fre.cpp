...
// 当 两个对象相加时是没有顺序要求的，但要重载 + 让其与一个数字相加则有顺序要求，可以通过加一个友元函数使另一个顺序的输入合法
#include <iostream>
using namespace std;
class A{
    private:
...
int main(){
        ...
        a2.display();
        int m = 1;
        a3 = a1 + a2; // 可以交换顺序，相当于 a3 = a1.operator+(a2);
        a3.display();
...
class Box{
    ...
    public:
        ...
        // 重载 + 运算符，用于把两个 Box 对象相加，因为其是全局函数，对应的参数个数是2，当重载的运算符函数是全局函数时，需要在类中将该函数声明为友元
...
// C++ 关系运算符重载
// C++ 语言支持各种关系运算符（<, >, <=, >=, == 等等），他们可用于比较 C++ 内置的数据类型
